#include "CRoundTree.h"

// Initialize the initial fruit size and game object data
CRoundTree::CRoundTree(const IGameObject::SGameData& data) {
...
void CRoundTree::attach(unsigned int location, IFruit * pFruit) {
	pFruit->setScale(this->mGameData->scale / this->mFruitSize);

	glm::vec3 low(
		-(this->mGameData->scale / 6.f),
		(this->mGameData->scale / 2.f),
		-(this->mGameData->scale / 6.f)
	);
...
// Getters
const IGameObject::SGameData& CRoundTree::getData() {
	return *this->mGameData;
}

const IGameObject::SGameData& CRoundTree::getFruitData(unsigned int fruitID) {
	return this->mFruits[fruitID]->getData();
}

unsigned int CRoundTree::getNumberOfFruits() {
	return this->mskNumberOfFruits;
}

// Grow the fruits attached to the tree until they reach maturity
void CRoundTree::growFruit() {
