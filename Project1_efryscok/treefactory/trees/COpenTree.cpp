...
COpenTree::COpenTree(const IGameObject::SGameData& data) {
...
void COpenTree::attach(unsigned int location, IFruit * pFruit) {
	pFruit->setScale(this->mGameData->scale / this->mFruitSize);

	glm::vec3 low(
		-(this->mGameData->scale / 4.f),
...
// Getters
const IGameObject::SGameData& COpenTree::getData() {
	return *this->mGameData;
}

const IGameObject::SGameData& COpenTree::getFruitData(unsigned int fruitID) {
	return this->mFruits[fruitID]->getData();
}

unsigned int COpenTree::getNumberOfFruits() {
	return this->mskNumberOfFruits;
}

// Grow the fruits attached to the tree until they reach maturity
void COpenTree::growFruit() {
